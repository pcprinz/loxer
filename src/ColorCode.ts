...
export function highlightColor(text: string, color?: string): string {
    if (color) {
        ...
        return bCol(Math.round(rgb.red()), Math.round(rgb.green()), Math.round(rgb.blue())) + text + ANSI_CODE.Reset;
    } else {
...
export function getServiceColor(color: string, alpha: number = 1) {
    ...
    return fCol(Math.round(rgb.red() * alpha), Math.round(rgb.green() * alpha), Math.round(rgb.blue() * alpha));
}
