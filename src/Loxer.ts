...
import { DEFAULT_MODULES, ensureError, filterDef, is, isError, isNES, isNumber, LoxerError } from './Helpers';
...
class LoxerInstance implements LoxerType {
    ...
    getModuleLevel(moduleId: string) {
        const level = this._dev ? this._modules[moduleId]?.develLevel : this._modules[moduleId]?.prodLevel;
        ...
        if (!is(openLox)) {
            return {
                add: (message: string, item?: any) => {
                    Loxer.internalError(
                        new LoxerError('add() on a not (anymore) existing Lox. MESSAGE: ' + message),
                        id,
                ...
                close: (message: string, item?: any) => {
                    Loxer.internalError(
                        new LoxerError('close() on a not (anymore) existing Lox. MESSAGE: ' + message),
                        id,
                        this._loxes[id]?.moduleId,
                        item
                    );
                ...
                error: (error: ErrorType, item?: any) => {
                    Loxer.internalError(
                        ensureError(error, 'error() on a not (anymore) existing Lox. ERROR: '),
                        id,
                        this._loxes[id]?.moduleId,
                        item
                    );
            ...
            return {
                ...
                error: (error: ErrorType, item?: any) => {
                    Loxer.internalError(error, openLox.id, this._loxes[openLox.id]?.moduleId, item);
                },
            };
        }
    }
    ...
    private appendLox(type: LoxType, openLox: Lox, message: string, item?: any) {
        ...
        const fixedLevel = type === 'single' ? (Math.max(level, this._level ?? level) as LogLevelType) : level;
        this.switchOutput(
            new Lox({
                id,
                highlighted: this._highlighted,
    ...
    private removeFromBuffer(log: Lox) {
        if (log.type === 'close') {
            ...
            let done = false;
            while (!done) {
                if (this._openLoxIdBuffer.length > 0 && !this._openLoxIdBuffer[this._openLoxIdBuffer.length - 1]) {
    ...
    private generateErrorLox(lox: Lox, error: Error): ErrorLox {
        ...
        errorLox.openLoxes = filterDef(openLoxes).map((errorLoxLog) => this.generateOutputLox(errorLoxLog));
        ...
        if (this._dev) {
            if (this._callbacks?.devError) {
                ...
                if (this._config?.disableColors) {
                    ...
                    console.log(errorLox.colored.moduleText + errorLox.colored.box + errorLox.colored.message);
                }
            }
    ...
    private devOut(outputLox: OutputLox) {
        if (this._callbacks?.devLog) {
            ...
            // colored option
            const { box, message, moduleText, timeText } = this._config?.disableColors ? outputLox.colored : outputLox;
    ...
    private generateOutputLox(lox: Lox): OutputLox {
        let coloredMessage = lox.highlighted ? highlightColor(lox.message, this._config.highlightColor) : lox.message;
        ...
        if (!outputLox.hidden) {
            switch (lox.type) {
                ...
                case 'close':
                    ...
                    outputLox.colored.message = this._highlighted ? coloredMessage : closeLogColor(lox.message);
                    break;
                case 'single':
                    outputLox.setBox(this.getOfLogBox(lox));
            }
    ...
    private getOpenLogBox(lox: Lox) {
        ...
        for (const openLoxId of this._openLoxIdBuffer) {
            ...
            cBox += openLoxId ? itemColor + BoxLayouts[this._config.boxLayoutStyle!].vertical + ANSI_CODE.Reset : ' ';
            box += openLoxId ? BoxLayouts[this._config.boxLayoutStyle!].vertical : ' ';
        }
        ...
        cBox +=
            ...
            ' ';
        box +=
            BoxLayouts[this._config.boxLayoutStyle!].openEdge + BoxLayouts[this._config.boxLayoutStyle!].openEnd + ' ';
        ...
        for (const openLoxLogId of this._openLoxIdBuffer) {
            ...
            if (!found) {
                if (openLoxLogId !== lox.id) {
                    ...
                    cBox += openLoxLogId
                        ? itemColor + BoxLayouts[this._config.boxLayoutStyle!].vertical + ANSI_CODE.Reset
                        : ' ';
                    box += openLoxLogId ? BoxLayouts[this._config.boxLayoutStyle!].vertical : ' ';
                } else {
                    // print occurrence
                    const occ =
                        lox.type === 'close'
                ...
                cBox += openLoxLogId
                    ? itemColor + BoxLayouts[this._config.boxLayoutStyle!].cross + ANSI_CODE.Reset
                    : mColor + BoxLayouts[this._config.boxLayoutStyle!].horizontal + ANSI_CODE.Reset;
                box += openLoxLogId
